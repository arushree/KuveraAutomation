...
public abstract class Control implements IControl {
	...
	protected static void throwControlCreationException(String name, String msg, Exception e) throws Exception {
		String emsg = String.format("Failure in %s. Exception: %s", msg, e.getMessage());
		logger.debug(emsg);
		throw new Exception("|||" + name + " is not Found|||" + emsg);
	}
	...
	protected void throwControlActionException(Exception e , String functionName) throws Exception {
		String emsg = String.format("Issue in %s for %s. Exception: %s", JavaUtils.getCaller(),
				this.getControlBasicInfoString(), e.getMessage());
		this.getAgent().takeSnapShot();
		logger.error(emsg);
		e.printStackTrace();
		throw new Exception("|||" + functionName + " not performed on " + getName() + "|||");
	}

	 protected static void throwControlCreationExceptionWithCustomMsg(String
	 myMsg, String msg, Exception e)
	 throws Exception {
	 logger.debug(msg);
	 Assert.fail("|||" + myMsg + ".|||" + msg);
	 }
	...
	public static IControl createControl(IPage page, String name, Identifier by) throws Exception {
		...
		try {
			...
			throwControlCreationException(name, msg, e);
		}
